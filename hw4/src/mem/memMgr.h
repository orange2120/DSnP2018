...
class MemBlock
{
    ...
    bool getMem(size_t t, T *&ret)
    {
        ...
        //              | <- memory to get -> |
        // | _begin |...|       <- t ->       |...| _end |
        //              ^_ptr -> current pos to check size is enough
        if (t + _ptr > _end)
            return false;
...
// Make it a private class;
// Only friend to MemMgr;
// *** A MemRecycleList only store one size of data ***
template <class T>
...
#define S sizeof(T)
  ...
  private:
    ...
    size_t getArraySize(size_t t) const
    {
        assert(t % SIZE_T == 0);
        assert(t >= S);
        // *** NOTE: memory size = n*obj_size+SIZE_T ***
        return (t - SIZE_T) / S;
    }
    ...
    T *getMem(size_t t)
    {
        ...
        MemRecycleList<T> *rec = getMemRecycleList(n);

        if (rec->_first != NULL)
        {
            // When found ,pop out it from the list
            ret = rec->popFront();
        }
        ...
        else
        {
            if (!_activeBlock->getMem(t, ret))
            {
                ...
                // If space not enough ,new a block directly
                if (rn >= S)
                {
                    getMemRecycleList(getArraySize(rn))->pushFront(ret);
#ifdef MEM_DEBUG
                    cout << "Recycling " << ret << " to _recycleList[" << getArraySize(rn) << "]\n";
#endif // MEM_DEBUG
                }
